...
CREATE TABLE IF NOT EXISTS `consultas_guardadas` (
  ...
  `nombre`        VARCHAR(190)    NOT NULL,           -- único, legible
  `sql_texto`     MEDIUMTEXT      NOT NULL,           -- la SQL completa
  `descripcion`   VARCHAR(500)    NULL,               -- opcional
  `tabla`         VARCHAR(128)    NULL,               -- tabla principal (si aplica)
  `etiquetas`     VARCHAR(255)    NULL,               -- p.ej. "reportes,ventas"
  `favorito`      TINYINT(1)      NOT NULL DEFAULT 0, -- 0/1
  `ejecuciones`   INT UNSIGNED    NOT NULL DEFAULT 0, -- contador
  `ultimo_run_at` DATETIME        NULL,               -- última ejecución
  `creado_en`     TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `actualizado_en`TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_consultas_guardadas_nombre` (`nombre`),
  KEY `idx_consultas_guardadas_tabla` (`tabla`),
  KEY `idx_consultas_guardadas_ultimo_run` (`ultimo_run_at`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8
  COLLATE=utf8_spanish_ci;
--
-- Insertar o actualizar por nombre
DELIMITER $$
CREATE PROCEDURE `sp_consulta_guardar`(
  IN p_nombre VARCHAR(190),
  IN p_sql    MEDIUMTEXT,
...
BEGIN
  INSERT INTO consultas_guardadas (nombre, sql_texto, descripcion, tabla, etiquetas, favorito)
  VALUES (p_nombre, p_sql, p_desc, p_tabla, p_tags, IFNULL(p_fav,0))
  ON DUPLICATE KEY UPDATE
    sql_texto   = VALUES(sql_texto),
...
-- Eliminar por nombre
DELIMITER $$
CREATE PROCEDURE `sp_consulta_eliminar`(IN p_nombre VARCHAR(190))
BEGIN
  DELETE FROM consultas_guardadas WHERE nombre = p_nombre;
END$$
DELIMITER ;

-- Obtener por nombre
DELIMITER $$
CREATE PROCEDURE `sp_consulta_obtener`(IN p_nombre VARCHAR(190))
BEGIN
  SELECT id, nombre, sql_texto, descripcion, tabla, etiquetas, favorito,
         ejecuciones, ultimo_run_at, creado_en, actualizado_en
  FROM consultas_guardadas
  WHERE nombre = p_nombre;
END$$
DELIMITER ;

-- Listar (ideal para poblar tu listbox)
DELIMITER $$
...
-- Marcar ejecución (suma contador y fecha)
DELIMITER $$
CREATE PROCEDURE `sp_consulta_marcar_ejecucion`(IN p_nombre VARCHAR(190))
BEGIN
...
DELIMITER $$
CREATE PROCEDURE sp_consulta_guardar_hc(
  IN  p_nombre VARCHAR(190),
  IN  p_sql    MEDIUMTEXT,
  IN  p_desc   VARCHAR(500),
  IN  p_tabla  VARCHAR(128),
  IN  p_tags   VARCHAR(255),
  IN  p_fav    TINYINT,
  OUT p_was_update TINYINT
)
BEGIN
  DECLARE v_exists TINYINT DEFAULT 0;
  SELECT EXISTS(SELECT 1 FROM consultas_guardadas WHERE nombre=p_nombre) INTO v_exists;
  ...
  SET p_was_update = v_exists;
END$$
DELIMITER ;

ALTER TABLE consultas_guardadas
  ADD COLUMN owner_id BIGINT UNSIGNED NULL AFTER id;

CREATE UNIQUE INDEX uq_consultas_guardadas_owner_nombre
  ON consultas_guardadas (owner_id, nombre);
